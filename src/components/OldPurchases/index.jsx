...
const OldPurchases = () => {
  ...
  const deletePurchase = (id) => {
    const filtredPurchases = oldPurchases.filter(
      (purchase) => purchase.id !== id,
    );
    setOldPurchase(filtredPurchases);
  };

  const generalExpenses = oldPurchases.reduce(
    (acc, newPurchase) => acc + parseFloat(newPurchase.cash),
    0,
  );

  return (
    <>
      <Example data={oldPurchases} />
      <LastPurchases
        key={oldPurchases.id}
